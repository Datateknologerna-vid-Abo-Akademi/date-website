...
response = client.list_objects(
    Bucket=settings.AWS_STORAGE_BUCKET_NAME, Prefix=settings.PRIVATE_MEDIA_LOCATION + "/", Delimiter="/"
)
...
for year in year_list:
    response = client.list_objects(
        Bucket=settings.AWS_STORAGE_BUCKET_NAME, Prefix=settings.PRIVATE_MEDIA_LOCATION + f"/{year}/", Delimiter="/"
    )
    album_year = int(year)
