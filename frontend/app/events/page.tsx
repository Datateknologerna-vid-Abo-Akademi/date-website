...
const RENDER_REFERENCE_TIME = Date.now();
...
function formatDay(value: string) {
  ...
  return date.toLocaleDateString("sv-FI", { day: "2-digit" });
}

function formatMonth(value: string) {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return "";
  return date.toLocaleDateString("sv-FI", { month: "short" });
}

function formatTime(value: string) {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return "";
  return date.toLocaleTimeString("sv-FI", { hour: "2-digit", minute: "2-digit" });
}

function formatWeekday(value: string) {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return "";
  return date.toLocaleDateString("sv-FI", { weekday: "short" });
}
...
export default async function EventsPage() {
  ...
  const upcomingEvents = events.filter(
    (event) => new Date(event.event_date_end).getTime() >= RENDER_REFERENCE_TIME,
  );
  const pastEvents = events
    .filter((event) => new Date(event.event_date_end).getTime() < RENDER_REFERENCE_TIME)
    .sort((left, right) => new Date(right.event_date_start).getTime() - new Date(left.event_date_start).getTime());

  return (
    <div className="page-shell events-index-page">
      <section className="hero compact">
        <p className="eyebrow">Events</p>
        <h1>Kommande handelser</h1>
      </section>

      <section className="panel events-index-panel">
        <div className="events-index-upcoming">
              ...
              <div key={event.slug} className="card-group event-card text-light mb-2">
                <div className="card mb-0 p-1">
                  <div className="card-body py-1">
                    <div className="row">
                      <div className="col-4 col-sm-2 events-index-date">
                      ...
                      <div className="col-6 col-sm-2 events-index-meta">
                        <ul className="list-group my-0 pl-1">
                          ...
                          <li className="list-inline-item">
                            <i className="fas fa-users" /> {event.sign_up_max_participants || "-"}
                          </li>
                        </ul>
                      </div>
