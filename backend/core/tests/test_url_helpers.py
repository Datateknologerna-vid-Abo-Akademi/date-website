...
class UrlHelpersTests(SimpleTestCase):
    ...
    def test_optional_include_returns_route_when_app_enabled(self):
        with patch.object(helpers.settings, "LEGACY_TEMPLATE_ROUTES_ENABLED", True), patch.object(
            helpers.settings, "INSTALLED_APPS", ["news"]
        ):
    ...
    def test_optional_members_includes_without_auth_urls(self):
        with patch.object(helpers.settings, "LEGACY_TEMPLATE_ROUTES_ENABLED", True), patch.object(
            helpers.settings, "INSTALLED_APPS", ["members"]
        ):
