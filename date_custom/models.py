...
class MembershipSignupRequest(models.Model):
    """Fullständigt namn, Födelsedag(ÅÅÅÅ-MM-DD), Matrikelnummer, Gatuadress, Postnummer, Postort, Telefonnummer, E-Post, Hemsida, Närmaste anhörig(Namn + telefonnummer), Processorhastighet"""
    full_name = models.CharField(
        _('Fullständigt namn'), max_length=100)
    birth_date = models.DateField(_('Födelsedag (ÅÅÅÅ-MM-DD)'))
    ...
    website = models.URLField(_('Hemsida'), blank=True, null=True)
    next_of_kin = models.CharField(
        _('Närmaste anhörig (Namn + telefonnummer)'), max_length=200)
    processor_speed = models.CharField(_('Processorhastighet'), max_length=50)
