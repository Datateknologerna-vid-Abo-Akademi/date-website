...
class CommentModelTests(TestCase):
    ...
    # Test 1. Test that our comment does not contain traces of nuts
    def test_comment_nut_trace(self):
        blog_post = Blog(title="Test post", pub_date=timezone.now(), body="Lorem ipsum")


        comment_with_nuts = Comment(user="Test user", blog=blog_post, pub_date=timezone.now(), text="this string contains nuts")
        comment_with_apples = Comment(user="Test user", blog=blog_post, pub_date=timezone.now(), text="this string contains apples")
        
        self.assertIs(comment_with_nuts.contains_traces_of_nuts(), True)
        self.assertIs(comment_with_apples.contains_traces_of_nuts(), False)
...
class BlogModelTests(TestCase):
    ...
    def test_blog_summary(self):
        ...
        # Test 2. Return if the given object was published recently
    def test_blog_published_recently(self):
        # 30 days in the future = Not published yet.
