...
- `archive.0008_remove_picture_collection_delete_examcollection_and_more` removes the old models from Django state after copying data.
- `archive.0009_drop_legacy_split_data` drops the copied legacy picture collection rows and the old `archive_picture` table after the replacement rows exist in `gallery`.
