...
- Two-page mode treats the first page as a cover (alone on the right) and, for PDFs with an even page count > 2, the last page as a back cover (alone on the left). PDFs with an odd page count don't get a back-cover view — the final spread `(N-1, N)` already shows the last page. 2-page PDFs render as a single `(1, 2)` spread and 1-page PDFs render the single page centered (no cover / back-cover concept applies). `normalizeTargetPage` in `eventListeners.js` snaps page-jump / End-key targets to valid spread positions, and `navigateNext` / `updateNavButtons` bail when the current spread already covers the whole document.
