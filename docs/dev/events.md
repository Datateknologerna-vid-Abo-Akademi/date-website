...
- Registration question names must be non-empty, unique per event, and must not collide with built-in signup or avec fields. Multiple-choice options are trimmed and must contain at least one unique value. These checks run server-side in both admin themes.
