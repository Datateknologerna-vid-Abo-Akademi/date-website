...
- `PROJECT_NAME` selects the active association/site variant (`date`, `kk`, `biocum`, `demo`, `pulterit`, `sf`, `impuls`, ...).
...
- Each release tag also publishes GitHub Release notes, which are the release history for the project. The release workflow groups merged PRs by the conventional-commit prefix of their squash-merge title (`feat:`, `fix:`, `docs:`, dependency scopes, maintenance types), so PRs need no release-category labels; PRs labeled `ignore-for-release` are left out.
