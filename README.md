...
- Each release tag also publishes GitHub Release notes, which are the release history for the project. The release workflow groups merged PRs by their release-category labels when present (`bug`, `feature`, `docs`, `dependencies`, `chore`, `refactor`) and otherwise by the conventional-commit prefix of their squash-merge title (`feat:`, `fix:`, `docs:`, dependency scopes, maintenance types), so PRs do not need labels; PRs labeled `ignore-for-release` are left out.
