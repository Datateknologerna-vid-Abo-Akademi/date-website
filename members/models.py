...
class Member(AbstractBaseUser, PermissionsMixin):
    username = models.CharField(
        _('Användarnamn'), unique=True, max_length=20, blank=False)
    email = models.EmailField(
        _('E-postadress'), unique=True, blank=True, null=True)
    ...
    def get_active_subscription(self):
        all_subscriptions = SubscriptionPayment.objects.filter(
            member=self).exclude(date_expires__lt=timezone.now())
...
class MembershipType(models.Model):
    ...
    permission_profile = models.IntegerField(
        _('Behörighetsprofil'), choices=PERMISSION_PROFILES, default=FRESHMAN)
...
class Subscription(models.Model):
    ...
    renewal_period = models.IntegerField(
        _('Förnyelseperiod'), blank=True, null=True)
    price = models.DecimalField(
        _('Pris'), decimal_places=2, max_digits=9, blank=False)
...
class SubscriptionPayment(models.Model):
    ...
    subscription = models.ForeignKey(
        'members.Subscription', on_delete=models.CASCADE)
    date_paid = models.DateField(
        _('Betald'), default=timezone.now, blank=False)
    date_expires = models.DateField(
        _('Upphör'), default=None, blank=True, null=True)
    amount_paid = models.DecimalField(
        _('Betald summa'), max_digits=9, decimal_places=2, null=True, blank=True)
...
class Functionary(models.Model):
    ...
    functionary_role = models.ForeignKey(
        FunctionaryRole, on_delete=models.CASCADE)
